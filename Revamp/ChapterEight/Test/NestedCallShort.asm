//  This file enables me test out the proposed asm for nested function call and return VM commands with an abridged VM Command
...
// For testing the asm code , I created a new VM function that is short and can allow me fully test the function-call-and-return statement

//  NOTE: This function has the same name (Sys.main) as the one above. I also replaced the one above with this one below in the Sys.vm file in this Test folder only to allow me test everything easily
//  The main Sys.main function above has another call inside and it will spoil my test as I need to try and test only one nested call for now before attempting multiple nested calls
...
//  push retAddrLabel   //  Push the ROM address where execution will commence for this caller function. This ROM address is the next address from this current execution hence A+1
