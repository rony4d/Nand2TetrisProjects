...
#define SYMBOL_ASM "/Users/ugochukwu/Desktop/rony/ComputerBasics/ProjectFiles/ChapterSix/iteration.asm"
#define SYMBOL_ASM_TEMP "/Users/ugochukwu/Desktop/rony/ComputerBasics/ProjectFiles/ChapterSix/iteration_temp.asm"
...
char * parse_a_instruction(char *instruction);
void parse_c_instruction(char *instruction, char *binary_result);
void parse_instruction(FILE *src_file);
void build_c_instruction();
void generate_symbol_table(FILE *src_file);
void extract_label_symbol(FILE *src_file);
void extract_variable_symbol(FILE *src_file);
...
Dictionary *a_inst_predefined_symbol_table;
Dictionary *a_inst_label_symbol_table;
Dictionary *a_inst_variable_symbol_table;
Dictionary *a_inst_symbol_table;
/**
...
void parse_instruction(FILE *src_file){
    ...
    while (fgets(instructions,MAX_BUFF,src_file) != NULL)
    {
        ...
        if (instructions[0] == '@')
        {
            ...
            char binary_result[BINARY_RESULT] = {0};
            char *a_binary_result= parse_a_instruction(instructions);
            printf("Binary Result: %s \n", a_binary_result);
        }
        else if(instructions[0] == '('){
            //Label is found and should be ignored, it has been processed and has been added to 
            //the appropriate general symbol and label symbol table
            printf("Label %s ignored \n", instructions);
        }
        else
        {
...
/**
 * In generating the a instruction symbol table, we will perform 3 operations:
 * 1. Initialization: Add the predefined symbols in the hack language to the symbol table
 * 2. First Pass: Iterate through the src_file, add all lables if any to the symbol table
 * 3. Second Pass: Iterate through the src_file the second time, add all variables if any to the symbol table
*/
void generate_symbol_table(FILE *src_file){
    
    /*  1. Initialization: Add the predefined symbols in the hack language to the symbol table. Also create
    redundant predefined symbol table */
    a_inst_symbol_table = build_a_inst_symbol_table();
    a_inst_predefined_symbol_table = build_a_inst_predefined_symbol_table();

    /*  2. First Pass: Iterate through the src_file, add all lables if any to the symbol table */
    extract_label_symbol(src_file);
    rewind(src_file);   // rewind file before we start to search again.
    /*   3. Second Pass: Iterate through the src_file, add all variable symbols */
    extract_variable_symbol(src_file);
    
}

/**
 * @brief extracts all labels (xxx) from the file 
*/
void extract_label_symbol(FILE *src_file){
    /* create redundant a_inst_label_symbol table to hold label symbols separately, 
    the label symbols will also be in the general symbol table
    */
    a_inst_label_symbol_table = DictCreate();
    ...
    while (fgets(instructions,MAX_BUFF,src_file) != NULL)
    {
        char label[BINARY_RESULT] = {0};
        char binary_result[BINARY_RESULT] = {0};
        char *instructions_result = strip_whitespace(instructions);
        
        //check the first character on the line, eat character if it is has (xxx)
        char ch = *(instructions_result++);

        if (ch == '(' )
        {
            /* we found first character showing label
               let us keep eating till will find the next terminating charcter ')'
             */
            
            while(ch != ')'){
                ch = *(instructions_result++);
                if(ch != ')')
                    strncat(label,&ch,1);
            }

            /*we found the terminating char ')'. Add the label to dictionary. NOTE, this label should point to the
            next memory location, so we do lince_count+1 */
            convert_decimal_to_binary(line_count+1,binary_result);
            DictInsert(a_inst_symbol_table,label,binary_result);
            DictInsert(a_inst_label_symbol_table,label,binary_result); // create redundant entry
            printf("Label-> Key: %s, Value: %s \n", label, DictSearch(a_inst_symbol_table,label));
        }
...
/**
 * @brief extracts all variables 
 * 1. Check any instruction that starts with @, if the next character is not a digit
 * 2. Check if that symbol exists in the a_inst_symbol table
 * 3. If it the symbol does not exist add it to the symbole table and variable_symbol_table for redundancy purposes
*/
void extract_variable_symbol(FILE *src_file){
    /* create redundant a_inst_variable_symbol table to hold variable symbols separately, 
    the variable symbols will also be in the general symbol table
    */

    a_inst_variable_symbol_table = DictCreate();


    char instruction[MAX_BUFF] = {0};
    int line_count = 0;
    int variable_memory_address_start = WORD_SIZE;

    rewind(src_file);       //point from beginning of file

    while (fgets(instruction,MAX_BUFF,src_file) != NULL)
    {
        char binary_result[BINARY_RESULT] = {0};
        ...
        if (instruction[0] == '@')
        {
            ...
            //check if the next character after @ is a character
            if (isalpha(instruction[1]))
            {
                /* Symbol A-instruction */
                
                //copy the rest of the characters into a new string
                char new_a_inst[WORD_SIZE] = {0}; // We just need a string big enough to hold the variable
                //TODO: reduce time by attempting to eat the first character instead of looping
                ...
                char *new_a_inst_trimmed = strip_whitespace(new_a_inst);
                /* check whether symbol exists in label symbol table or predefined symbol table */
                const char *label_symbol = DictSearch(a_inst_label_symbol_table,new_a_inst_trimmed);
                const char *predefined_symbol = DictSearch(a_inst_predefined_symbol_table,new_a_inst_trimmed);

                if ((label_symbol == 0) && (predefined_symbol == 0))
                {
                    
                    /* label symbol and predefined symbol does not exist, hence we found a variable symbol */
                                  
                    /* check if the variable symbol has been added to the variable symbol table before */
                    if (DictSearch(a_inst_variable_symbol_table,new_a_inst_trimmed) == 0)
                    {
                        /* variable symbol has not be inserted into variable table and general symbol table yet, so insert*/
                        convert_decimal_to_binary(variable_memory_address_start,binary_result);
                        DictInsert(a_inst_symbol_table,new_a_inst_trimmed,binary_result);           // entry into general symbol table
                        DictInsert(a_inst_variable_symbol_table,new_a_inst_trimmed,binary_result);  // redundant entry into variable symbol table

                        // printf("var %s, memory address: %d \n", new_a_inst_trimmed, variable_memory_address_start);
                        printf("Variable-> Key: %s, Value: %s \n", new_a_inst_trimmed, DictSearch(a_inst_symbol_table,new_a_inst_trimmed));
                        variable_memory_address_start++;
                    }
...
char * parse_a_instruction(char *instruction){
    ...
    {
        /* Symbol A-instruction */
        /* get the binary result by passing instruction as key to the general symbol dictionary and it will return the binary result
           and you are home and dry :D
         */
        ...
        binary_result = DictSearch(a_inst_symbol_table,new_a_inst_trimmed);
...
int main(int argc, char *argv[]){
    ...
    */
    ...
    src_file = fopen(SYMBOL_ASM, "r");          // opens source file for reading
    tmp_file = fopen(SYMBOL_ASM_TEMP, "w");    // opens tmp file for writing, creates file if it does not exist
    ...
    // Test Label Extraction and Variable Extraction
    //src_file = fopen(SYMBOL_ASM, "r");
    // a_inst_symbol_table = build_a_inst_symbol_table(); // create the general symbol table
    // a_inst_predefined_symbol_table = build_a_inst_predefined_symbol_table(); // build predefined symbol table

    // extract_label_symbol(src_file);
    // extract_variable_symbol(src_file);
    // generate_symbol_table(src_file);
    // fclose(src_file);

    // Parse Instruction Live Main Function

    printf("\n\n\n\n Parsing Symbolic instructions in assembly file \n\n");
